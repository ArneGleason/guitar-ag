...
int main (int argc, char* argv[])
{
    ...
    for (auto i = 1; i < argc; ++i)
    {
        ...
        else if (argument == "--legacy-additive-pick-mix" && hasValue)
        {
            legacyAdditivePickMix = juce::jlimit (0.0f, 1.0f, juce::String (argv[++i]).getFloatValue());
        }
        else if (argument == "--legacy-pick-texture-density" && hasValue)
        {
            legacyPickTextureDensity = juce::jlimit (1.0f, 6.0f, juce::String (argv[++i]).getFloatValue());
        }
        ...
        else if (argument == "--pick-bite" && hasValue)
        {
            pickBite = juce::String (argv[++i]).getFloatValue();
        }
        ...
        else if (argument == "--strum-speed" && hasValue)
        {
            strumSpeed = juce::String (argv[++i]).getFloatValue();
        }
        else if (argument == "--strum-balance" && hasValue)
        {
            strumBalance = juce::String (argv[++i]).getFloatValue();
        }
    ...
    engine.setStrumSpeed (strumSpeed);
    engine.setStrumBalance (strumBalance);

...
The offline renderer uses the same `AudioEngine` as the VST3 processor, so it is useful for rapid DSP iteration. The default legacy-engine tone is the production EG-089 recipe. Pass `--legacy-tone-recipe previous` to reproduce the former EG-088 production tone; the individual `--legacy-*` ablation flags can still override that named recipe for research. It does not test DAW/plugin-host behavior, plugin scanning, or UI behavior.

Optional arguments include `--gain`, `--legacy-tone-recipe`, `--input-octave`, `--input-transpose`, `--sustain`, `--pick-stiffness`, `--pick-texture`, `--pick-bite`, `--pick-stroke`, `--strum-speed`, `--strum-balance`, `--player-feel`, `--player-feel-recovery`, `--player-feel-reset-at`, `--palm-mute`, `--harmonic-touch`, `--string-age`, `--bridge-intonation`, `--aftertouch`, `--channel-pressure`, `--cc74`, `--mpe-mode`, `--mpe-bend-range`, `--mpe-pressure-amount`, `--mpe-cc74-amount`, `--pickup-position`, `--pickup-model`, `--legato-articulation`, `--amp-feedback`, `--feedback-return-distorted`, `--slide-squeak-up`, and `--slide-squeak-down`. `--input-octave daw` matches DAWs that label MIDI note 52 as guitar E2; `--input-octave midi` leaves MIDI note 40 as guitar E2. `--slide-squeak` remains an alias for upward squeak. `--gain` can align the offline WAV level with a DAW export if the DAW project has track/output gain applied.
...
- New plugin instances should open as `v0.3.1 / StringVoice EG-089 ModalFormantTone`.
...
- The editor identity line should show the current model label, for example `StringVoice EG-089 ModalFormantTone`.

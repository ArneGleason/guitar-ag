...
Optional arguments include `--gain`, `--sustain`, `--pick-stiffness`, `--pick-texture`, `--palm-mute`, `--harmonic-touch`, `--string-age`, `--pickup-position`, and `--pickup-model`. `--gain` can align the offline WAV level with a DAW export if the DAW project has track/output gain applied.
